...
await testStep('The liked post does not move to the top', async () => {
	...
	).not.toContainElement(removeFavoriteButton)
	const secondPost = posts[1]
	expect(
		secondPost,
		'The second post should have a add favorite button',
	).toContainElement(removeFavoriteButton)
})
