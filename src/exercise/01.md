...
- You can initialize a default value for a function's parameter if it isn't specified by the consumer.

```js
function Greeting({ initialName = '' }) { }
```

- For `<input>` elements, consider binding the value of the initial state like this to synchronize changes between the initial state and the later, changing state:
