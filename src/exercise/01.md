...
## Stefanos Notes
The React hook, React.useState() function is used in this example to store nd change some data while interactin with some components. For the initial ex. we change the const name='' to an array consnt [name, setName]= React.useState('').

The React.useState returns the name variable in the fist array index and the setName function for the second indexed value in the array. the setName function is used to place the value within the handler class, e.g. within handler function -> 

setName(event.target.value)

For the extra credit the JSX of the Greeting had the prop of initialName set to "Kody"
the initial name is passed as a variable 'initialName within {} into the argument of the greeting function.

Once passed the variable is set as the default name in the set state, e.g.
React.setState(initialName)
