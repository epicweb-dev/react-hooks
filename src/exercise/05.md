...
Using the useRef to get a reference of a DOM node. the syntax od the use ref is as fallows:

const foo = React.useRef, this within a function so as to call it.

it is then used with a .useEffect with the lazy state initialization as such:

React.useEffect(()=>{
  const bar = foo.current
}, []) // rememberig to implement the dependecies array when using the useEffect hook,
in this case since there is no dependecies and we want the effect to run all the time we leave it clear.
