...
using the useEffect with the callback linked to the pokemonName state, which is used in the dependecies array, the initial pokemon state for the name is set to null so as 
to have the loading screen or the Fallback be activated. 

We use the react use state to initiate the pokemon and setPokemon befor the use effect.

using the use effect with the lazy state initialization.

The status of the state is cheched with an if statement, that then returns different types of values depending on the pokemonName submitted.

for the extra credit 4 e are to use an object to store the states of the hook so that it can be used with such a call.:

```shell
setState({status: 'resolved', pokemon})
```

Extra credit 4 notes, using the Error Boundery component (not a function call but a class for now).


Extra credit 6 using the react-error-boundery npm, such usage must be impirted with the following sinapsis

```shell
import {ErrorBoundery} from 'react-error-boundery'
```

Extra credit 7 using react-error-boundery's resetErrorBoundery

The resetErrorBoundary is passed to the ErrorBoundary component when the rey again button is clicked.
we then call the reset custom function tht will reste the pokemon name to an empty string as to not 
be stuck in the errorthe next re-render.

using the resetKeys prop with the [pokemonName] within the tag in the app return section within the ErrorBoundary tag.
